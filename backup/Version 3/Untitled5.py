...
def find_optimal_clusters(word_boxes):
    
    vertical_gaps=calculate_vertical_gaps(word_boxes)
    vertical_gaps_array = np.array(vertical_gaps).reshape(-1, 1)


    # Compute within-cluster dispersion for different values of k
    Wk = []
    for k in range(1, len(word_boxes)):
        kmeans = KMeans(n_clusters=k, random_state=0)
        kmeans.fit(vertical_gaps_array)
        inertia = kmeans.inertia_
        if inertia > 1e-10:  # Exclude very small inertia values
            Wk.append(inertia)
    ...
    # Generate reference datasets and compute their within-cluster dispersion
    B = 20 # Number of reference datasets
    Wkb = np.zeros((len(Wk), B))
    for b in range(B):
        # Generate reference data
        reference_data = np.random.uniform(
            np.min(vertical_gaps_array), np.max(vertical_gaps_array), size=vertical_gaps_array.shape)
        for i, inertia in enumerate(Wk):
            kmeans = KMeans(n_clusters=i + 1, random_state=0)
            kmeans.fit(reference_data)
            Wkb[i, b] = kmeans.inertia_

    # Compute the gap statistic
    gap = np.log(np.mean(Wkb, axis=1)) - np.log(Wk)

    # Find the optimal number of clusters that maximizes the gap statistic
    optimal_k = np.argmax(gap) + 2
   
    
    if (optimal_k<=len(word_boxes)):
         return optimal_k
         
    
    else:
        return len(word_boxes)

def calculate_threshold(word_boxes):
 # Convert vertical_gaps to a NumPy 
    vertical_gaps=calculate_vertical_gaps(word_boxes)
    vertical_gaps_array = np.array(vertical_gaps).reshape(-1, 1)
  
    # Apply K-means clustering with the optimal number of clusters
    kmeans = KMeans(n_clusters=find_optimal_clusters(word_boxes), random_state=0)
    kmeans.fit(vertical_gaps_array)
...
async def imageToText(file: UploadFile = File(...)):
    try:
        ...
        try:
            ...
            print(f"line_boxes:{line_boxes}")
        except Exception:
            return {"message": "Processing Error2"}
